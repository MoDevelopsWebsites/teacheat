...
const RealtimeAnswersCard: React.FC = () => {
  ...
  return (
    <Card className="relative w-full max-w-md h-64 bg-uiLightBlue dark:bg-uiDarkBlue rounded-xl shadow-lg overflow-hidden border border-gray-200 dark:border-gray-800 flex items-center justify-center">
      ...
      <div className="absolute inset-0 p-2 sm:p-4 text-gray-300 dark:text-gray-700 text-xs font-mono opacity-50 overflow-hidden"> {/* Adjusted padding for mobile */}
        ...
        <div className="absolute inset-0 z-0" style={{
          ...
          {backgroundWords.map((word, i) => (
            <span key={i} className="inline-block mx-1 sm:mx-2 my-0.5 sm:my-1 text-gray-300 dark:text-gray-700 opacity-70 text-[0.6rem] sm:text-xs"> {/* Adjusted text size and spacing for mobile */}
              {word}
            </span>
          ))}
        </div>
      ...
      {/* Command + Return keys */}
      <CardContent className="relative z-20 flex items-center justify-center p-0 space-x-1 sm:space-x-2"> {/* Adjusted spacing for mobile */}
        <div className="flex items-center justify-center w-24 h-12 sm:w-28 sm:h-16 bg-uiLightBlue dark:bg-uiDarkBlue rounded-lg shadow-md border border-gray-200 dark:border-gray-600 text-gray-800 dark:text-gray-200 text-xs sm:text-sm font-semibold
                    cursor-pointer transition-all duration-200 hover:scale-105 active:scale-95 active:shadow-inner"> {/* Adjusted size and text size for mobile */}
          <Command className="h-3 w-3 sm:h-4 sm:w-4 mr-1" /> command
        </div>
        <span className="text-gray-700 dark:text-gray-300 text-base sm:text-lg font-bold">+</span> {/* Adjusted text size for mobile */}
        <div className="flex items-center justify-center w-24 h-12 sm:w-28 sm:h-16 bg-uiLightBlue dark:bg-uiDarkBlue rounded-lg shadow-md border border-gray-200 dark:border-gray-600 text-gray-800 dark:text-gray-200 text-xs sm:text-sm font-semibold
                    cursor-pointer transition-all duration-200 hover:scale-105 active:scale-95 active:shadow-inner"> {/* Adjusted size and text size for mobile */}
