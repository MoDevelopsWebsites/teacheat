...
const Participant: React.FC<ParticipantProps> = ({ name, email, role, isOwner, avatarUrl }) => (
  <div className="flex items-center justify-between py-1.5 sm:py-2">
    <div className="flex items-center space-x-2 sm:space-x-3">
      <Avatar className="h-8 w-8 sm:h-9 sm:w-9">
        {avatarUrl ? (
          ...
          <AvatarFallback className="bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-sm sm:text-base">
    ...
    <div className="flex items-center space-x-1 text-undetectable-text-muted text-xs sm:text-sm">
      <span>{role}</span>
...
const MeetingParticipantsCard: React.FC = () => {
  return (
    <Card className="w-full max-w-sm bg-undetectable-card-background rounded-xl shadow-lg border border-undetectable-card-border overflow-hidden">
      <CardHeader className="p-3 sm:p-4 border-b border-undetectable-card-border flex flex-row items-center justify-between">
        <CardTitle className="text-base sm:text-lg font-semibold text-undetectable-text-primary">
      ...
      <CardContent className="p-3 sm:p-4 space-y-1 sm:space-y-2">
        ...
        <div className="flex items-center justify-center py-1.5 sm:py-2 text-undetectable-text-muted text-sm">
