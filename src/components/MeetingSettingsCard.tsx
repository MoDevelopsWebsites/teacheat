...
const SettingItem: React.FC<SettingItemProps> = ({ icon, label, status, checked, className }) => (
  <div className={cn(
    ...
    <div className="relative w-14 h-14 sm:w-16 sm:h-16 bg-uiLightBlue dark:bg-uiDarkBlue rounded-xl flex items-center justify-center shadow-sm border border-gray-200 dark:border-gray-700">
      {icon}
      {checked && (
        <CheckCircle2 className="absolute -top-2 -right-2 h-5 w-5 sm:h-6 sm:w-6 text-blue-500 bg-white dark:bg-gray-900 rounded-full" fill="currentColor" />
      )}
    </div>
...
const MeetingSettingsCard: React.FC = () => {
  return (
    <Card className="w-full max-w-md bg-uiLightBlue dark:bg-uiDarkBlue rounded-xl shadow-lg border border-gray-200 dark:border-gray-800">
      ...
      <CardContent className="grid grid-cols-2 gap-4 p-4 sm:gap-6 sm:p-6">
        <SettingItem
          icon={<Mic className="h-7 w-7 sm:h-8 sm:w-8 text-blue-500" />}
          label="Microphone"
          status="Connected"
          checked={true}
        />
        <SettingItem
          icon={<Volume2 className="h-7 w-7 sm:h-8 sm:w-8 text-blue-500" />}
          label="System Audio"
          status="Streaming"
          checked={true}
        />
        <SettingItem
          icon={<Waves className="h-7 w-7 sm:h-8 sm:w-8 text-blue-500" />}
          label="Voice Events"
          status="Capturing"
          checked={true}
        />
        <SettingItem
          icon={<Wifi className="h-7 w-7 sm:h-8 sm:w-8 text-blue-500" />}
          label="Network"
          status="Active"
          checked={true}
        />
